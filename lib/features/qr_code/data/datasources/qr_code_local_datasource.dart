...
abstract class QrCodeLocalDataSource {
  ...
  Future<String> exportQrCodeAsImage(QrCodeModel qrCode, String format, int size);
...
class QrCodeLocalDataSourceImpl implements QrCodeLocalDataSource {
  ...
  }) : _dbHelper = dbHelper, _prefs = prefs;
  ...
  @override
  Future<String> exportQrCodeAsImage(QrCodeModel qrCode, String format, int size) async {
    try {
  ...
  Future<Uint8List> _generateQrImage(QrCodeModel qrCode, int size) async {
    try {
      ...
      final paint = Paint()
        ..color = _hexToColor(qrCode.backgroundColor);
