...
class UserProfileRepository {
  ...
  Future<Either<Failure, Map<String, dynamic>>> syncUserProfile(String userId) async {
    try {
      // Check connectivity
      final connectivityResult = await _connectivity.checkConnectivity();
      final hasConnectivity = connectivityResult.isNotEmpty && 
        ...
        try {
          ...
          await _dbHelper.update(
            'sync_queue',
            {
              ...
              'status': (item['retry_count'] as int) >= 5 ? 'failed' : 'pending',
            },
            where: 'id = ?',
            whereArgs: [item['id']],
          );
  ...
  Future<Map<String, dynamic>> _pushLocalChangesToRemote(Map<String, dynamic> userData) async {
    try {
      final token = await _tokenManager.getAccessToken();
      if (token == null) {
      ...
      if (response.statusCode == 200) {
        ...
        // If the response is wrapped in a data field, extract it
        final updatedData = responseData is Map<String, dynamic> && responseData.containsKey('data')
  ...
  Future<void> _addToSyncQueue(String userId, Map<String, dynamic> userData) async {
    try {
